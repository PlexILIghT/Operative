...
def render_hp(health):
    ...
    data.screen.blit(text, (data.screen_width * 0.7, data.screen_height * 0.88))
...
def render_ammo():
    ...
    data.screen.blit(text, (data.screen_width * 0.7, data.screen_height * 0.8))
...
def display_time(start_time):
    ...
    data.screen.blit(text, (data.screen_width // 15, data.screen_height // 14))

def render_enemy_amount():
    text = data.font.render(f"ENEMIES LEFT {data.cur_amount_of_enemies}/{len(data.enemies)}", True, "white")
    data.screen.blit(text, (data.screen_width // 15, data.screen_height // 8))
